...
相信你已经看到了头文件内不仅仅是将 MCLOG 复制到 mclog.h 文件中，文件内还存在着另三个 MCLOG_H 宏将 MCLOG 的内容包裹住，这个 MCLOG_H 是保护宏，它的功能就是将头文件的内容进行保护，让同一个 .cpp 文件只能导入一次头文件，如果重复导入 mclog.h 则直接退出，因为重复导入会引起重命名问题，导致程序编译失败  
设置头文件保护 **编程规范** 的重要一步  
三个 MCLOG_H 宏的功能逻辑是，如果没有定义 MCLOG_H 宏，则定义这个宏，并导入头文件内容，如果已经定义了这个宏则直接跳过这段代码    
MCLOG_H 与 mclog.h 在命名上是高度关联的，每一个头文件都需要有自己的保护宏，这意味着我们不同让文件名重复，更不能让保护宏重复，负责会编译错误    
你可能还会看到使用 #pragma 代替保护宏的代码，因为 #pragma 更简洁，但是我不推荐你使用它，当然这只是个人使用习惯  
...
// 打印
["hello world!": hello world!] [argc: 2] 
["hello world!": hello world!] [argc: 2]  
~~~
