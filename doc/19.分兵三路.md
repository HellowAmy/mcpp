...
标记多线程函数是 **编程规范** 的重要一步   
我习惯打一个 th 的标识在函数中，这样可以让我更谨慎的对待这些函数，不至于犯下一些愚蠢的操作，你要记住在多线程函数中，每一次对外部数据的修改都是危险的   
...
除了两个锁相互嵌套之外，单锁的递归上锁也会造成死锁，不过在递归中上锁也是神奇，我愿称为愚蠢的代码，因为很多时候换一种设计方式就能避免不必要的麻烦   
...
其中 compare_exchange_weak 函数来检查一开始的共享数据，如果相同则交换，不同则放弃，这是函数是安全交换共享数据的关键，因为在多线程中修改共享数据是危险的，而 std::memory_order_release 为更新内存顺序的标记，它声明了所有内存顺序中，到这一步为止都是正确的，它确保了修改的共享数据是最新的  
