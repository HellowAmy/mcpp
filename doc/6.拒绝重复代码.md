...
~~~
template <typename T>
void fun_template(T arg)
{
}
~~~

我们来看一个简单的模板函数创建，fun_template 函数就是一个带有 T 类型的模板函数，T 就是不确定的类型，T 可以是任何类型，只要符合函数的运行即可  
你会看到声明 T 为模板参数的一行代码 template \<typename T\> 这一行代码便是需要创建一个 T 类型，否则 fun_template 函数无法使用 T arg 这个变量，注意 T 的声明位置，实在函数名前面的，C++的大多数声明都需要在函数名前面  
类型 T 的写法是大写的，这是STL标准库的命名习惯，从而影响了很多模板类型的声明，我认为这个 T 大写还不错，或许类型名应该起一个更有实际意义的名称  
template \<typename T\> 也有 template \<class T\> 的写法，但是我不推荐你这样写，因为使用 typename 更能显示出这是在声明一个模板类型，是更准确的类型声明，而不是创建一个类名之类的与类有关的误解    
使用准确类型声明是 **编程规范** 的重要一步  
